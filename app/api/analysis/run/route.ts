...
export async function POST(request: NextRequest) {
  ...
  const jar = await cookies(); const session = jar.get("maithri_session")?.value ?? crypto.randomUUID();
  ...
  try {
    ...
    if (!jar.get("maithri_session")) response.cookies.set("maithri_session", session, { httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", path: "/", maxAge: 60 * 60 * 24 * 90 }); return response;
  } catch { return NextResponse.json({ error: "The analysis could not be generated. Please try again." }, { status: 500 }); }
}
